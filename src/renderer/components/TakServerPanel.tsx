...
export default function TakServerPanel() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 p-4">
      ...
      {/* Status */}
      <div className="bg-secondary-dark rounded-lg p-4 flex items-center gap-4">
      ...
      <div className="bg-secondary-dark rounded-lg p-4 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            <label htmlFor={`${id}-port`} className="block text-xs text-gray-400 mb-1">
              Port (1024–65535)
            </label>
            <input
              id={`${id}-port`}
              ...
              className="w-32 px-2 py-1.5 bg-deep-black border border-gray-600 rounded text-gray-200 text-sm focus:border-brand-green focus:outline-none disabled:opacity-50"
            />
            {localPort !== '' && !portValid && (
              <p className="text-xs text-red-400 mt-1">Port must be 1024–65535</p>
            )}
          </div>

          <div>
            <label htmlFor={`${id}-name`} className="block text-xs text-gray-400 mb-1">
              Server Name
            </label>
            <input
              id={`${id}-name`}
              ...
              disabled={status.running || isLoading}
              className="w-full max-w-xs px-2 py-1.5 bg-deep-black border border-gray-600 rounded text-gray-200 text-sm focus:border-brand-green focus:outline-none disabled:opacity-50"
            />
          </div>
        ...
        <div className="flex gap-2 pt-2">
          {!status.running ? (
            <button
              ...
              className="px-4 py-2 bg-brand-green hover:bg-brand-green/90 text-black font-medium rounded-lg text-sm transition-colors disabled:opacity-50"
            >
            ...
            <button
              onClick={handleStop}
              disabled={isLoading}
              className="px-4 py-2 bg-red-700 hover:bg-red-600 text-white font-medium rounded-lg text-sm transition-colors disabled:opacity-50"
            >
      ...
      <div className="bg-secondary-dark rounded-lg p-4 space-y-3">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-secondary-dark border border-gray-600 hover:border-gray-500 text-gray-200 font-medium rounded-lg text-sm transition-colors disabled:opacity-50"
          >
      ...
      <div className="bg-secondary-dark rounded-lg p-4 space-y-3">
        ...
        <button
          ...
          disabled={isLoading}
          className="px-4 py-2 bg-secondary-dark border border-gray-600 hover:border-red-800 text-gray-300 hover:text-red-300 font-medium rounded-lg text-sm transition-colors disabled:opacity-50"
        >
